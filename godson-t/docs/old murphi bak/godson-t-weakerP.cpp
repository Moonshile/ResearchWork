/******************************
  Program "godson-t-weakerP.m" compiled by "Caching Murphi Release 5.4.9"
...
#define PROTOCOL_NAME "godson-t-weakerP"
#define BITS_IN_WORLD 352
#define ALIGN
...
class mu_1_LOCK
{
 public:
  ...
  mu_1_TYPE_ADDR mu_criticalRegion;
  mu_0_boolean mu_inCriticalRegion;
...
friend int CompareWeight(mu_1_LOCK& a, mu_1_LOCK& b)
  {
    ...
    if (w!=0) return w;
    w = CompareWeight(a.mu_criticalRegion, b.mu_criticalRegion);
    if (w!=0) return w;
    w = CompareWeight(a.mu_inCriticalRegion, b.mu_inCriticalRegion);
    if (w!=0) return w;
  return 0;
}
friend int Compare(mu_1_LOCK& a, mu_1_LOCK& b)
  {
    ...
    if (w!=0) return w;
    w = Compare(a.mu_criticalRegion, b.mu_criticalRegion);
    if (w!=0) return w;
    w = Compare(a.mu_inCriticalRegion, b.mu_inCriticalRegion);
    if (w!=0) return w;
  return 0;
}
  virtual void Permute(PermSet& Perm, int i);
  ...
  virtual void MultisetSort()
  {
    ...
    mu_criticalRegion.MultisetSort();
    mu_inCriticalRegion.MultisetSort();
  }
  void print_statistic()
  {
    mu_owner.print_statistic();
    mu_beUsed.print_statistic();
    mu_criticalRegion.print_statistic();
    mu_inCriticalRegion.print_statistic();
  }
  void clear() {
    mu_owner.clear();
    mu_beUsed.clear();
    mu_criticalRegion.clear();
    mu_inCriticalRegion.clear();
 };
  void undefine() {
    mu_owner.undefine();
    mu_beUsed.undefine();
    mu_criticalRegion.undefine();
    mu_inCriticalRegion.undefine();
 };
  void reset() {
    mu_owner.reset();
    mu_beUsed.reset();
    mu_criticalRegion.reset();
    mu_inCriticalRegion.reset();
 };
  void print() {
    mu_owner.print();
    mu_beUsed.print();
    mu_criticalRegion.print();
    mu_inCriticalRegion.print();
  };
  void print_diff(state *prevstate) {
    mu_owner.print_diff(prevstate);
    mu_beUsed.print_diff(prevstate);
    mu_criticalRegion.print_diff(prevstate);
    mu_inCriticalRegion.print_diff(prevstate);
  };
  void to_state(state *thestate) {
    mu_owner.to_state(thestate);
    mu_beUsed.to_state(thestate);
    mu_criticalRegion.to_state(thestate);
    mu_inCriticalRegion.to_state(thestate);
  };
virtual bool isundefined() { Error.Error("Checking undefinedness of a non-base type"); return TRUE;}
virtual bool ismember() { Error.Error("Checking membership for a non-base type"); return TRUE;}
  mu_1_LOCK& operator= (const mu_1_LOCK& from) {
    ...
    mu_criticalRegion.value(from.mu_criticalRegion.value());
    mu_inCriticalRegion.value(from.mu_inCriticalRegion.value());
    return *this;
  };
};
...
void mu_1_LOCK::set_self(const char *n, int os)
{
  ...
  if (name) mu_criticalRegion.set_self_2(name, ".criticalRegion", os + 16 ); else mu_criticalRegion.set_self_2(NULL, NULL, 0);
  if (name) mu_inCriticalRegion.set_self_2(name, ".inCriticalRegion", os + 24 ); else mu_inCriticalRegion.set_self_2(NULL, NULL, 0);
}
...
/*** Variable declaration ***/
mu_1__type_4 mu_node("node",80);

/*** Variable declaration ***/
mu_1_TYPE_NODE mu_curNode("curNode",296);

/*** Variable declaration ***/
mu_1_TYPE_CACHE mu_curCache("curCache",304);

/*** Variable declaration ***/
mu_1_TYPE_ADDR mu_curMemory("curMemory",312);

/*** Variable declaration ***/
mu_1_TYPE_DATA mu_curData("curData",320);

/*** Variable declaration ***/
mu_1_TYPE_LOCK mu_curLock("curLock",328);

/*** Variable declaration ***/
mu_1_REPLACE_STAGE mu_replace("replace",336);

/*** Variable declaration ***/
mu_1_REPLACE_RULE mu_repRule("repRule",344);
...
mu_lock[mu_l].mu_inCriticalRegion = mu_false;
  };
...
mu_lock[mu_l].mu_criticalRegion = mu_a;
mu_lock[mu_l].mu_inCriticalRegion = mu_true;
  };
...
void SetNextEnabledRule(unsigned & what_rule)
{
  ...
  if (what_rule>=12 && what_rule<36)
    { R2.NextRule(what_rule);
      if (what_rule<36) return; }
  if (what_rule>=36 && what_rule<84)
    { R3.NextRule(what_rule);
      if (what_rule<84) return; }
  if (what_rule>=84 && what_rule<108)
    { R4.NextRule(what_rule);
      if (what_rule<108) return; }
  if (what_rule>=108 && what_rule<120)
    { R5.NextRule(what_rule);
      if (what_rule<120) return; }
  if (what_rule>=120 && what_rule<144)
    { R6.NextRule(what_rule);
      if (what_rule<144) return; }
  if (what_rule>=144 && what_rule<156)
    { R7.NextRule(what_rule);
      if (what_rule<156) return; }
  if (what_rule>=156 && what_rule<180)
    { R8.NextRule(what_rule);
      if (what_rule<180) return; }
  if (what_rule>=180 && what_rule<204)
    { R9.NextRule(what_rule);
      if (what_rule<204) return; }
  if (what_rule>=204 && what_rule<228)
    { R10.NextRule(what_rule);
      if (what_rule<228) return; }
  if (what_rule>=228 && what_rule<240)
    { R11.NextRule(what_rule);
      if (what_rule<240) return; }
  if (what_rule>=240 && what_rule<264)
    { R12.NextRule(what_rule);
      if (what_rule<264) return; }
  if (what_rule>=264 && what_rule<276)
    { R13.NextRule(what_rule);
      if (what_rule<276) return; }
  if (what_rule>=276 && what_rule<282)
    { R14.NextRule(what_rule);
      if (what_rule<282) return; }
  if (what_rule>=282 && what_rule<294)
    { R15.NextRule(what_rule);
      if (what_rule<294) return; }
  if (what_rule>=294 && what_rule<300)
    { R16.NextRule(what_rule);
      if (what_rule<300) return; }
  if (what_rule>=300 && what_rule<306)
    { R17.NextRule(what_rule);
      if (what_rule<306) return; }
  if (what_rule>=306 && what_rule<308)
    { R18.NextRule(what_rule);
      if (what_rule<308) return; }
  if (what_rule>=308 && what_rule<311)
    { R19.NextRule(what_rule);
      if (what_rule<311) return; }
  if (what_rule>=311 && what_rule<317)
    { R20.NextRule(what_rule);
      if (what_rule<317) return; }
  if (what_rule>=317 && what_rule<320)
...
bool Condition(unsigned r)
{
  ...
  if (r>=12 && r<=35) return R2.Condition(r-12);
  if (r>=36 && r<=83) return R3.Condition(r-36);
  if (r>=84 && r<=107) return R4.Condition(r-84);
  if (r>=108 && r<=119) return R5.Condition(r-108);
  if (r>=120 && r<=143) return R6.Condition(r-120);
  if (r>=144 && r<=155) return R7.Condition(r-144);
  if (r>=156 && r<=179) return R8.Condition(r-156);
  if (r>=180 && r<=203) return R9.Condition(r-180);
  if (r>=204 && r<=227) return R10.Condition(r-204);
  if (r>=228 && r<=239) return R11.Condition(r-228);
  if (r>=240 && r<=263) return R12.Condition(r-240);
  if (r>=264 && r<=275) return R13.Condition(r-264);
  if (r>=276 && r<=281) return R14.Condition(r-276);
  if (r>=282 && r<=293) return R15.Condition(r-282);
  if (r>=294 && r<=299) return R16.Condition(r-294);
  if (r>=300 && r<=305) return R17.Condition(r-300);
  if (r>=306 && r<=307) return R18.Condition(r-306);
  if (r>=308 && r<=310) return R19.Condition(r-308);
  if (r>=311 && r<=316) return R20.Condition(r-311);
  if (r>=317 && r<=319) return R21.Condition(r-317);
...
void Code(unsigned r)
{
  ...
  if (r>=12 && r<=35) { R2.Code(r-12); return; } 
  if (r>=36 && r<=83) { R3.Code(r-36); return; } 
  if (r>=84 && r<=107) { R4.Code(r-84); return; } 
  if (r>=108 && r<=119) { R5.Code(r-108); return; } 
  if (r>=120 && r<=143) { R6.Code(r-120); return; } 
  if (r>=144 && r<=155) { R7.Code(r-144); return; } 
  if (r>=156 && r<=179) { R8.Code(r-156); return; } 
  if (r>=180 && r<=203) { R9.Code(r-180); return; } 
  if (r>=204 && r<=227) { R10.Code(r-204); return; } 
  if (r>=228 && r<=239) { R11.Code(r-228); return; } 
  if (r>=240 && r<=263) { R12.Code(r-240); return; } 
  if (r>=264 && r<=275) { R13.Code(r-264); return; } 
  if (r>=276 && r<=281) { R14.Code(r-276); return; } 
  if (r>=282 && r<=293) { R15.Code(r-282); return; } 
  if (r>=294 && r<=299) { R16.Code(r-294); return; } 
  if (r>=300 && r<=305) { R17.Code(r-300); return; } 
  if (r>=306 && r<=307) { R18.Code(r-306); return; } 
  if (r>=308 && r<=310) { R19.Code(r-308); return; } 
  if (r>=311 && r<=316) { R20.Code(r-311); return; } 
  if (r>=317 && r<=319) { R21.Code(r-317); return; } 
}
int Priority(unsigned short r)
{
  if (r<=5) { return R0.Priority(); } 
  if (r>=6 && r<=11) { return R1.Priority(); } 
  if (r>=12 && r<=35) { return R2.Priority(); } 
  if (r>=36 && r<=83) { return R3.Priority(); } 
  if (r>=84 && r<=107) { return R4.Priority(); } 
  if (r>=108 && r<=119) { return R5.Priority(); } 
  if (r>=120 && r<=143) { return R6.Priority(); } 
  if (r>=144 && r<=155) { return R7.Priority(); } 
  if (r>=156 && r<=179) { return R8.Priority(); } 
  if (r>=180 && r<=203) { return R9.Priority(); } 
  if (r>=204 && r<=227) { return R10.Priority(); } 
  if (r>=228 && r<=239) { return R11.Priority(); } 
  if (r>=240 && r<=263) { return R12.Priority(); } 
  if (r>=264 && r<=275) { return R13.Priority(); } 
  if (r>=276 && r<=281) { return R14.Priority(); } 
  if (r>=282 && r<=293) { return R15.Priority(); } 
  if (r>=294 && r<=299) { return R16.Priority(); } 
  if (r>=300 && r<=305) { return R17.Priority(); } 
  if (r>=306 && r<=307) { return R18.Priority(); } 
  if (r>=308 && r<=310) { return R19.Priority(); } 
  if (r>=311 && r<=316) { return R20.Priority(); } 
  if (r>=317 && r<=319) { return R21.Priority(); } 
return 0;}
char * Name(unsigned r)
{
  if (r<=5) return R0.Name(r-0);
  if (r>=6 && r<=11) return R1.Name(r-6);
  if (r>=12 && r<=35) return R2.Name(r-12);
  if (r>=36 && r<=83) return R3.Name(r-36);
  if (r>=84 && r<=107) return R4.Name(r-84);
  if (r>=108 && r<=119) return R5.Name(r-108);
  if (r>=120 && r<=143) return R6.Name(r-120);
  if (r>=144 && r<=155) return R7.Name(r-144);
  if (r>=156 && r<=179) return R8.Name(r-156);
  if (r>=180 && r<=203) return R9.Name(r-180);
  if (r>=204 && r<=227) return R10.Name(r-204);
  if (r>=228 && r<=239) return R11.Name(r-228);
  if (r>=240 && r<=263) return R12.Name(r-240);
  if (r>=264 && r<=275) return R13.Name(r-264);
  if (r>=276 && r<=281) return R14.Name(r-276);
  if (r>=282 && r<=293) return R15.Name(r-282);
  if (r>=294 && r<=299) return R16.Name(r-294);
  if (r>=300 && r<=305) return R17.Name(r-300);
  if (r>=306 && r<=307) return R18.Name(r-306);
  if (r>=308 && r<=310) return R19.Name(r-308);
  if (r>=311 && r<=316) return R20.Name(r-311);
  if (r>=317 && r<=319) return R21.Name(r-317);
  return NULL;
}
};
const unsigned numrules = 320;

/********************
  parameter
 ********************/
#define RULES_IN_WORLD 320
...
bool mu__boolexpr187;
  if (!((mu_replace) == (mu_NON))) mu__boolexpr187 = FALSE ;
  else {
  mu__boolexpr187 = (mu_node[mu_i].mu_hasLock) ; 
}
  if (!(mu__boolexpr187)) mu__boolexpr186 = FALSE ;
  else {
  mu__boolexpr186 = (!(mu_node[mu_i].mu_firstRead[mu_a])) ; 
}
  if (!(mu__boolexpr186)) mu__boolexpr185 = FALSE ;
  else {
  mu__boolexpr185 = ((mu_node[mu_i].mu_cache[mu_j].mu_state) == (mu_VALID)) ; 
}
  if (!(mu__boolexpr185)) mu__boolexpr184 = FALSE ;
  else {
  mu__boolexpr184 = ((mu_node[mu_i].mu_cache[mu_j].mu_addr) == (mu_a)) ; 
}
...
bool mu__boolexpr190;
  if (!(mu_lock[mu_m].mu_inCriticalRegion)) mu__boolexpr190 = FALSE ;
  else {
  mu__boolexpr190 = (mu_lock[mu_n].mu_inCriticalRegion) ; 
}
  if (!(mu__boolexpr190)) mu__boolexpr189 = FALSE ;
  else {
  mu__boolexpr189 = ((mu_lock[mu_m].mu_criticalRegion) != (mu_lock[mu_n].mu_criticalRegion)) ; 
}
if ( !(mu__boolexpr189) )
...
bool mu__condition_194() // Condition for Rule "Coherence, a:1, j:1, i:1"
{
  return mu__invariant_181( 1, 1, 1 );
}

bool mu__condition_195() // Condition for Rule "Coherence, a:1, j:1, i:2"
{
  return mu__invariant_181( 1, 1, 2 );
}

bool mu__condition_196() // Condition for Rule "Coherence, a:1, j:1, i:3"
{
  return mu__invariant_181( 1, 1, 3 );
}

bool mu__condition_198() // Condition for Rule "Coherence, a:1, j:2, i:1"
{
  return mu__invariant_181( 1, 2, 1 );
}

bool mu__condition_199() // Condition for Rule "Coherence, a:1, j:2, i:2"
{
  return mu__invariant_181( 1, 2, 2 );
}

bool mu__condition_200() // Condition for Rule "Coherence, a:1, j:2, i:3"
{
  return mu__invariant_181( 1, 2, 3 );
}

bool mu__condition_203() // Condition for Rule "Coherence, a:2, j:1, i:1"
{
  return mu__invariant_181( 2, 1, 1 );
}

bool mu__condition_204() // Condition for Rule "Coherence, a:2, j:1, i:2"
{
  return mu__invariant_181( 2, 1, 2 );
}

bool mu__condition_205() // Condition for Rule "Coherence, a:2, j:1, i:3"
{
  return mu__invariant_181( 2, 1, 3 );
}

bool mu__condition_207() // Condition for Rule "Coherence, a:2, j:2, i:1"
{
  return mu__invariant_181( 2, 2, 1 );
}

bool mu__condition_208() // Condition for Rule "Coherence, a:2, j:2, i:2"
{
  return mu__invariant_181( 2, 2, 2 );
}

bool mu__condition_209() // Condition for Rule "Coherence, a:2, j:2, i:3"
{
...
int mu__invariant_210( const mu_1_TYPE_NODE &mu_i) // Invariant "DeadlockFree"
{
bool mu__boolexpr211;
bool mu__boolexpr212;
  if (!((mu_replace) == (mu_NON))) mu__boolexpr212 = FALSE ;
  else {
  mu__boolexpr212 = (mu_node[mu_i].mu_hasLock) ; 
}
...
bool mu__boolexpr215;
  if (!(mu_lock[mu_l].mu_beUsed)) mu__boolexpr215 = FALSE ;
  else {
  mu__boolexpr215 = ((mu_lock[mu_l].mu_owner) == (mu_i)) ; 
}
if ( (mu__boolexpr215) )
  { mu__quant214 = TRUE; break; }
};
};
  if (!(mu__quant214)) mu__boolexpr213 = FALSE ;
  else {
bool mu__quant216; 
mu__quant216 = TRUE;
{
...
bool mu__boolexpr220;
  if ((mu_m) == (mu_n)) mu__boolexpr220 = TRUE ;
  else {
  mu__boolexpr220 = (!(mu_lock[mu_m].mu_beUsed)) ; 
}
  if (mu__boolexpr220) mu__boolexpr219 = TRUE ;
  else {
  mu__boolexpr219 = (!(mu_lock[mu_n].mu_beUsed)) ; 
}
  if (mu__boolexpr219) mu__boolexpr218 = TRUE ;
  else {
  mu__boolexpr218 = ((mu_lock[mu_m].mu_owner) != (mu_i)) ; 
}
  if (mu__boolexpr218) mu__boolexpr217 = TRUE ;
  else {
  mu__boolexpr217 = ((mu_lock[mu_n].mu_owner) != (mu_i)) ; 
}
if ( !(mu__boolexpr217) )
  { mu__quant216 = FALSE; break; }
};
};
};
  mu__boolexpr213 = (mu__quant216) ; 
}
  mu__boolexpr211 = (mu__boolexpr213) ; 
}
return mu__boolexpr211;
};

bool mu__condition_222() // Condition for Rule "DeadlockFree, i:1"
{
  return mu__invariant_210( 1 );
}

bool mu__condition_223() // Condition for Rule "DeadlockFree, i:2"
{
  return mu__invariant_210( 2 );
}

bool mu__condition_224() // Condition for Rule "DeadlockFree, i:3"
{
  return mu__invariant_210( 3 );
}

/**** end rule declaration ****/

const rulerec invariants[] = {
{"DeadlockFree, i:3", &mu__condition_224, NULL, },
{"DeadlockFree, i:2", &mu__condition_223, NULL, },
{"DeadlockFree, i:1", &mu__condition_222, NULL, },
{"Coherence, a:2, j:2, i:3", &mu__condition_209, NULL, },
{"Coherence, a:2, j:2, i:2", &mu__condition_208, NULL, },
{"Coherence, a:2, j:2, i:1", &mu__condition_207, NULL, },
{"Coherence, a:2, j:1, i:3", &mu__condition_205, NULL, },
{"Coherence, a:2, j:1, i:2", &mu__condition_204, NULL, },
{"Coherence, a:2, j:1, i:1", &mu__condition_203, NULL, },
{"Coherence, a:1, j:2, i:3", &mu__condition_200, NULL, },
{"Coherence, a:1, j:2, i:2", &mu__condition_199, NULL, },
{"Coherence, a:1, j:2, i:1", &mu__condition_198, NULL, },
{"Coherence, a:1, j:1, i:3", &mu__condition_196, NULL, },
{"Coherence, a:1, j:1, i:2", &mu__condition_195, NULL, },
{"Coherence, a:1, j:1, i:1", &mu__condition_194, NULL, },
};
